...
void mpu9250_get_data(I2C_Handle *i2c, float *ax, float *ay, float *az, float *gx, float *gy, float *gz) {
	...
	int16_t nx = (float)(((nolla + rawData[0]) << 8) + rawData[1]);
	int16_t ny = (float)(((nolla + rawData[2]) << 8) + rawData[3]);
	int16_t nz = (float)(((nolla + rawData[4]) << 8) + rawData[5]);
	int16_t mx = (float)(((nolla + rawData[8]) << 8) + rawData[9]);
	int16_t my = (float)(((nolla + rawData[10]) << 8) + rawData[11]);
	int16_t mz = (float)(((nolla + rawData[12]) << 8) + rawData[13]);
